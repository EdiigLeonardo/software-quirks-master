import { Category, categoryLabels, categoryDescriptions, allCategories } from "@/data/questions";
...
const devopsCategories: Category[] = [
  "aws",
  "github-actions",
  "gcp",
  "azure",
  ...
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-sm">{categoryLabels[cat]}</p>
      ...
      <div
        className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? "border-primary bg-primary" : "border-muted-foreground/40"
          }`}
      >
        {isSelected && (
          <svg
...
export function QuizStart({
  ...
  const isAllSelected = selectedCategories.length === (allCategories?.length ?? 11);
